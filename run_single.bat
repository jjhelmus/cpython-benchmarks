...
REM run benchmarks
pyperformance run %RUN_ARGS% --output results.json > bench.log 2>&1
...
copy results.json "%result_dir%\results-%TIMESTAMP%.json"
copy bench.log "%result_dir%\log-%TIMESTAMP%.txt"
