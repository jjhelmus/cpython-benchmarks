...
# run benchmarks
pyperformance run --manifest ../custom/MANIFEST ${RUN_ARGS} --output results.json | tee bench.log
python -c "import sys; print(sys.version)" >> bench.log

# copy results and log
result_dir=../results/${1}
mkdir -p ${result_dir}
cp results.json ${result_dir}/custom-results-$(date "+%Y-%m-%dT%H%M%S").json
cp bench.log ${result_dir}/custom-log-$(date "+%Y-%m-%dT%H%M%S").txt
